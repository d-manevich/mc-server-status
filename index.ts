...
function parseServerStatus(res: PingResponse, prevStatus?: ServerStatus): ServerStatus {
    const {
        players: {max, sample = []},
    } = res;
    const online = sample
...
function isMinecraftServerAvailable(serverUrl: string, host: string, port: number | undefined) {
...
async function subscribe(msg: Message) {
    ...
    try {
        ...
        try {
            const isServerAvailable = await isMinecraftServerAvailable(url, host, port);
...
function onServerUpdate(url: string, err?: Error, res?: PingResponse) {
    ...
    if (
        currentServerStatusMessage !==
        CACHED_MESSAGES.get(url)
    ) {
