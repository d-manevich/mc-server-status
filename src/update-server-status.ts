...
function formatOfflinePlayer(player: PlayerStatus) {
    const formattedDuration = formatDistance(player.lastOnline, new Date(),  { addSuffix: true })
...
function getOfflineSection(offline: PlayerStatus[]) {
    const filteredOffline = offline
        .filter((p) => Math.abs(differenceInMinutes(p.lastOnline, new Date())) < THRESHOLD_TO_SHOW_OFFLINE_PLAYERS_MINS)
...
function getPlayerListSection(online: PlayerStatus[], offline: PlayerStatus[]) {
    return online.length || offline.length
        ? `${[getOnlineSection(online), getOfflineSection(offline)].filter(Boolean).join("\n")}`
        : "";
}
